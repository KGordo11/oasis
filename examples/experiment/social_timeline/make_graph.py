...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* The cost charts are generated by make_timing_charts.py against the
     scaling artifact's token names. Alias them here rather than re-rendering,
     so one generator serves both pages and they cannot drift apart. */
  #panel-cost { --measured: var(--follow); --corrected: var(--interact);
                --rule: var(--line); --ink: var(--fg);
                --sans: "IBM Plex Sans", system-ui, sans-serif;
                --mono: "IBM Plex Mono", monospace; }
  #panel-cost figure { margin:0 0 28px; }
  #panel-cost figcaption { font-size:12.5px; color:var(--muted);
                           margin-top:8px; line-height:1.5; max-width:66ch; }
  #panel-cost h3 { font-size:15px; margin:0 0 6px; }
...
const TABS = [['network','Network'], ['rounds','Rounds'],
              ...
              ['timeline','Timeline'], ['cost','Cost & scaling'],
              ['method','Method & integrity']];
...
COST_CHART_DIR = "data/charts"

COST_CAPTIONS = {
    "rounds": ("Cost per round, 36 agents",
               "Nine replicate runs; the band is one standard deviation. The "
               "first three rounds ramp, then it goes flat. The ramp is the "
               "agent's transcript filling the context window."),
    "rounds_by_agents": ("Cost per round, by world size",
                         "The ramp takes the same four rounds at every size. "
                         "Bigger worlds sit higher; they do not ramp for "
...
def cost_charts():
    """Inline the SVGs written by make_timing_charts.py.

    They are read rather than re-drawn so a single generator feeds both this
    page and the scaling write-up. Two generators drifting apart is how this
    project ended up publishing one run's timings under another run's name.
    """
    out = []
    for key in ("rounds", "rounds_by_agents", "agents"):
        path = os.path.join(COST_CHART_DIR, f"cost_by_{key}.svg")
        if not os.path.exists(path):
            continue
        title, caption = COST_CAPTIONS[key]
...
def main():
    ...
    html = html.replace("__COST_CHARTS__", cost_charts())
