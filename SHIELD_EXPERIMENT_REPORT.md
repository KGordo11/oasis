...
**Abstract.** In Simulation 2, our AI agents pushed back on the same false
...
the order, a result strong enough that we re-ran the experiment ten times
before trusting it. Full method, results, and limitations follow.

This experiment builds directly on Simulation 2
(`COUNTERFACTUAL_EXPERIMENT_REPORT.md`) — read that one first if you
haven't.
...
Before describing what *we* did, this section summarizes what *they* did,
so the connection is fully traceable rather than a loose inspiration.
...
directly relevant to our own Finding 3 in Section 8.

---

## 2. Why we built on this paper
...
| **Self-reflection mechanism** — compares the reranked list to the previous one, regenerates on mismatch to fight hallucination | Our validation step checks that the Shield's returned post-ID set exactly matches what was sent in, retries once on mismatch, and **fails open** (shows the plain feed) if it still doesn't match | Same safeguard, adapted for a live simulation that can't afford to stall or loop indefinitely waiting for a perfect answer |
| **i²Agent's dynamic memory** (Profile Generator + Dynamic Extractor, built from a user's feedback across many sessions) | **Not built.** | The paper's dynamic memory is explicitly built by accumulating one user's feedback *across multiple sessions over time*. OASIS agents only exist for the length of one simulation run — there is no persistent, cross-session history to build a dynamic memory from, so this piece of the paper's architecture doesn't have anything to attach to in our setup. |
| **InstructRec datasets** (Amazon Book/Movie, Goodreads, Yelp + synthetic instructions) | **Not built — we reused Simulation 2's existing dataset instead** (220 false claims from `counterfactual_36.json`) | We're testing herd behavior on misinformation, not product-recommendation ranking quality, so the paper's e-commerce datasets don't fit the question; Simulation 2 already had a dataset built for exactly this purpose |
| **Ranking-quality metrics** (HR@k, NDCG@3, MRR, FR@k, P-HR@3, P-MRR) | **Not used — we reused Simulation 2's keyword-based disagreement/correction classifier instead** | The paper's metrics all assume a single "correct" item exists to rank highly. There's no equivalent "correct answer" in our setup — what we measure instead is whether agents push back on a *false* claim, which the paper's metrics were never designed to capture |

**In one sentence:** we built the paper's base iAgent (Parser + Reranker +
Self-reflection) exactly as designed, deliberately left out the parts of
the architecture that require multi-session memory or a different kind of
dataset, and pointed the same core mechanism at a different question than
the paper asked — not "does hiding the platform's signal improve ranking
quality," but "does hiding the platform's signal change how much agents
push back on things that aren't true."

---

## 4. Where did this code come from — did we copy the paper, or write our own?

**Short answer: we wrote all of it ourselves.** The paper's own public
code (`github.com/agiresearch/iAgent`, referenced in Section 1) is built
for real recommendation datasets and a real ranking pipeline — it isn't
written for OASIS and wouldn't run here. What we took from the paper was
the *architecture*, mapped explicitly in Section 3, and we wrote a
completely new, original Python implementation of it, built specifically
to plug into our existing simulator.

**Files we created from scratch (all new, nothing like them existed
before):**

| File | What it does |
|---|---|
| `examples/experiment/reddit_simulation_counterfactual/shield_agent.py` | The Shield itself — the actual new code |
| `examples/experiment/reddit_simulation_counterfactual/reddit_simulation_shielded.py` | The script you run to start a shielded simulation |
...
**The one existing file we edited** (not created — this file already
existed from Simulation 2): `reddit_simulation_counterfactual.py`. Two
small edits were made to it: (1) already in place before this experiment,
swapping a hardcoded connection to the paper authors' private computer
cluster for a connection to your own free local AI model instead; (2)
during this experiment, adding a longer wait-time setting so a slow local
AI response doesn't get mistaken for a total failure. Neither edit touches
what the simulation actually *does* — how agents act, how posts are
scored, how many rounds run.

**We did not touch OASIS's own code at all.** OASIS is the simulator
...
circuits, we built an attachment that plugs into a controller port it
already has, and told the console "use my attachment instead of the
default one" with a single line of code
(`agents_generator.SocialAgent = ShieldedSocialAgent`). If our attachment
had a bug, the console underneath was never at risk. This is the exact
same approach Simulation 1 used for a different experiment, so it's a
pattern we'd already trusted before starting this one.
...
2. Before that feed becomes what the agent actually reads, we insert one
   extra step: a second AI call (the "Shield," using the same free local
   model, `llama3.1:8b`) — this is our version of the paper's Reranker —
...
3. The agent is shown **only** the Shield's re-ordered list with those
   sentences attached — the actual like/dislike numbers never make it into
   the text the agent reads. We didn't just tell the Shield to *ignore*
   the numbers; we physically removed them from what gets sent to the
   agent.
4. If the Shield's answer comes back broken (wrong post IDs, badly-formed
   text, or the network call fails outright) — this is our version of the
   paper's self-reflection check — it tries once more. If that also
   fails, it **gives up safely** — shows the agent the plain, unmodified
   feed instead of crashing the whole simulation. We call this "failing
   open," and it turned out to matter a lot (see Section 7).
...
## 7. Bugs we hit along the way — and why they mattered
...
ruined the results if we hadn't caught it before trusting the data.

**Bug 1 — the like/dislike numbers leaked through anyway.** Our settings
file said "show scores as a single combined number" instead of "show likes
and dislikes separately." The Shield's cleanup step only knew how to
remove the separate version — so the combined number slid right through,
completely undoing the whole point of the experiment. We caught this by
watching the simulation's live output as it ran, before it got far enough
to matter, and fixed the cleanup step to catch every possible version of
the number.

**Bug 2 — one slow network response crashed the entire simulation.** The
Shield knew how to recover from a *badly worded* answer from the AI model,
but not from the AI model *timing out* entirely. A single slow response
from one agent's Shield check crashed all 36 agents' simulation at once —
turning "fails safely, never crashes" from something we'd designed into
something that wasn't actually true yet. We caught this with a much
smaller practice run (2 rounds instead of 6, about 23 minutes instead of
an hour+) and fixed it so a slow or failed response now safely falls back
instead of crashing anything.

**Bug 3 — the AI model occasionally answered in a way our code didn't
...
quirk in how our code checked for missing information let this slip past
our safety checks and crash the program — an hour into what would have
been a successful run. We caught this with a second small practice run,
double-checked it with a focused test built specifically to recreate that
exact situation, and fixed the code to always double-check the type of
answer it got before using it.
...
practice run (2 rounds, ~20-25 minutes) first. Two of the three bugs never
would have shown up in a finished results file — you had to either watch
the simulation live or run it small enough to catch problems quickly. This
became a standing rule for the rest of the project.

---

## 8. What we found the first time we ran it
...
land almost exactly on the fake starting score we forced (−1.00 average),
shield or no shield. Expected, not surprising — this experiment was never
about whether votes themselves snowball; Simulation 2 already answered
that.
...
This finding is our own direct check of a related question in our domain
...
downstream agent actually acts on a correct Shield verdict once it's given
one, which is what we observe here.
...
Shield call, which we double-checked directly with an automated test that
fails loudly if a vote number ever slips through. But hiding that signal
didn't just remove herd behavior — in this run, it removed most of the
*skepticism* Simulation 2 had measured, without reliably replacing it with
independent accuracy. That's a real, if slightly unflattering, result.
...
safe-fallback design (Section 5, step 4 — our version of the paper's
self-reflection safeguard) trades a little bit of purity for the
simulation never crashing outright — which, given three separate
crash-causing bugs during development (Section 7), was clearly the right
trade.

---

## 10. Checking our own work: did the finding actually hold up?
...
We ran three more full simulations, then four more after that — eight
...
Shield was switched on at all. If we'd also tinkered with the Shield's
internal settings while adding more runs, we'd have no way to tell whether
a moved number came from the new condition or from the Shield behaving
differently — so we deliberately left two promising-looking improvements
(Section 12, items 1-2) completely untouched for this whole stretch of
testing.
...
control, not one noisy comparison, which is why we're comfortable calling
it out as the strongest finding in this whole experiment.
...
- **The comment counts behind every percentage are small** — 13 to 29
  ...
  move any of these numbers meaningfully. That's exactly why we kept
  re-running instead of trusting the first number.
...
- **We built the paper's base architecture (iAgent), not its extended
  version (i²Agent)**, and did not build the InstructRec datasets or
  formal ranking metrics — Section 3 explains exactly why each of those
  pieces doesn't map onto our setup. This experiment tests the paper's
  *mechanism*, not a reproduction of the paper's own reported numbers.
...
5. **Try the same experiment with only one personality repeated 36
   times**, instead of 36 different personalities. Right now, we can't
   fully tell apart "the fake vote caused this behavior" from "these 36
   particular made-up people happened to react this way" — using one
   repeated personality would isolate the first question from the second.
   Not started; would need a new set of agent data to be built first.
6. **Save this work properly** — none of the code, data, or this report
   has been formally saved into the project's permanent history yet.
7. Two open questions carried over from Simulation 2 that this experiment
   didn't touch: replacing the simple keyword search with an AI-graded
   score for more precision, and running the whole experiment at a much
   larger scale (hundreds or thousands of agents instead of 36) to see if
   a real vote-pile-on effect appears at that size.

---

## 13. One paragraph you could read out loud

> "I extended a misinformation experiment by building a content-quality
> 'Shield' that sits between the platform and each AI agent and hides the
> crowd's vote count before the agent ever sees it. The design is a direct
> adaptation of a real ACL 2025 paper, iAgent: LLM Agent as a Shield
> between User and Recommender Systems — I built their base architecture's
> three components (a parser, a reranker, and a self-reflection safeguard)
> inside my own simulator, and can point to exactly which piece of my code
> maps to which piece of their paper. Building it surfaced three real
> bugs, each caught by testing at a smaller scale before committing to an
> hour-plus full run. Once it worked, the finding surprised me: hiding the
> vote count didn't make agents better fact-checkers, it made them
> quieter — pushback on the same false claims dropped from 68% to roughly
> a quarter. I checked that finding nine more times across three more
> conditions before trusting it, and the most interesting result only
> showed up once I had enough data: hiding the vote count doesn't just
> weaken the original effect, it partially *flips* which condition gets
> the most pushback. That's a more specific and more surprising result
> than 'the shield worked,' and it only became visible because I kept
> re-running the experiment instead of trusting the first number."
